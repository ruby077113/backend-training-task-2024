...
// JavaScript 中的乘法運算符 (*) 要求參數都必須是數字。如果某一邊不是數字類型Js會嘗試進行類型轉換。
// "8" 是一個字串，在執行 k * l 時, JavaScript 會將"string" "8" 利用Number()轉換為"number" 8。
...
if (giftNum<=0) {
  console.log("抱歉，贈品沒啦，下次請早")
}else if (BobPrice >= giftPriceRule || BobIsVip) {
  console.Log（"客戶您好，您有符合贈品資格"）；
  giftNum--;
} else {
  console.log("抱歉，您未符合贈品資格");
}
...
if(coachIncome <= 100000){
  ...
  bonus=0.2;}
coachBonus = coachBonus +baseBonus* (1+bonus);
...
const {running, yoga, cycling} = activities;
calorieBurn=running.caloriePerMin*30+yoga.caloriePerMin*40+cycling.caloriePerMin*20;
